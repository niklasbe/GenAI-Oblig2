...
// Prompt template for generating AI travel listings
...
export async function generateListing(): Promise<Listing> {
	try {
		// Request AI-generated listing from OpenAI		
		const completion = await openai.chat.completions.create({
			model: "gpt-4o-mini",
			messages: [
				{
					// Set system role as travel listing expert
					role: "system",
					content: "You are an expert at creating unique and appealing travel listings."
				},
				{
					// Send the listing generation prompt
					role: "user",
		...
		const requiredFields: (keyof Listing)[] = [
			'title', 'location', 'description', 'propertyType',
			'keyFeatures', 'pricePerNight', 'idealFor'
		];
		...
		// Add fields that are not generated by the model (random ID and rating)
